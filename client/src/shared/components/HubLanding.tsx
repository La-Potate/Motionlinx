  ...
  return (
    ...
    >
      <button
        ...
        className="flex flex-col gap-4 text-left rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
        <span className="flex w-full items-start justify-between">
          {/* Shares a layoutId with the destination page's header mark, so the
              tile morphs across the navigation instead of popping in. */}
          <motion.span
      ...
      {tool.whatItDoes?.length ? (
        <ul className="flex flex-col gap-1">
          {tool.whatItDoes.slice(0, 2).map((item) => (
            <li key={item} className="flex items-start gap-1.5 text-xs text-foreground-muted">
              <Check
